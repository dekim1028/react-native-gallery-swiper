import PageList from "@daeun2351/react-native-page-list";
...
import ImageTransformer from "@daeun2351/react-native-image-transformer";
  ...
  constructor(props) {
    ...
    this.imageResponder = {
      ...
      onEnd: (evt, gestureState) => {
        this._galleryViewPager && this._galleryViewPager.resetFade();
        const currentImageTransformer = this.getCurrentImageTransformer();
        currentImageTransformer &&
    ...
    return (
      <ImageTransformer
        ...
        onPinchTransforming={(transform) => {
          onPinchTransforming && onPinchTransforming(transform, pageId);
        }}
        onPinchStartReached={(transform) => {
          onPinchStartReached && onPinchStartReached(transform, pageId);
        }}
        onPinchEndReached={(transform) => {
          onPinchEndReached && onPinchEndReached(transform, pageId);
        }}
        ...
        onSwipeUpReleased={(transform) => {
          onSwipeUpReleased && onSwipeUpReleased(transform, pageId);
        }}
        onSwipeDownReleased={(transform) => {
          onSwipeDownReleased && onSwipeDownReleased(transform, pageId);
        }}
        onDoubleTapStartReached={(transform) => {
          onDoubleTapStartReached && onDoubleTapStartReached(transform, pageId);
        }}
        onDoubleTapEndReached={(transform) => {
          onDoubleTapEndReached && onDoubleTapEndReached(transform, pageId);
        }}
        ref={(ref) => {
  ...
  render() {
    ...
    return (
      <PageList
        ...
        enableFlashList={this.props.enableFlashList}
      />
    );
  }
}
